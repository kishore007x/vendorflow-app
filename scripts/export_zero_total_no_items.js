...
(async()=>{
  const outPath = 'tmp-zero-total-no-items.csv';
  ...
  const rows = data.filter(o => !(o.order_items && o.order_items.length>0)).map(o=> ({ id:o.id, order_number:o.order_number||'', portal:o.portal||'', total_amount:o.total_amount==null? '': o.total_amount, created_at:o.created_at||'' }));
  const csv = ['id,order_number,portal,total_amount,created_at', ...rows.map(r => `${r.id},"${r.order_number}",${r.portal},${r.total_amount},${r.created_at}`)].join('\n');
